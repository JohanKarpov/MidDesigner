...
export function spawnOrder() {
    ...
    if (!state.chapter1Completed) return;
...
export function completeOrder(orderId) {
    ...
    // Award payout (with optional mini-game modifier + ai_neural stack bonus)
    ...
    const _contractorMult    = (_contractorTier > 0 && (order.taskType === 'contract' || order.taskCategory === 'contracts'))
        ...
        : 1;
    const _finalPayout  = Math.max(0, Math.round((order.realPayout + _minigenBonus) * _neuralMult * _contractorMult));
...
export function registerGenerationStep(isAutogen = false) {
    ...
    if (Date.now() < state.generationCooldownUntil) return;
...
function _chapterLevelCap() {
    if (!state.chapter1Completed) return 2;
    if (!state.chapter2Completed) return 4;
    return Infinity;
}
